...
.factory('TempoRealData', function($http, $q, TempoRealStorage) {
    ...
    success(function(d) {
        ...
        data = d.rss.channel.item.dadosevento;
        var dataJogoFull = d.rss.channel.item.data;
        var dataJogo = dataJogoFull.replace("-", "<br/>");;
       	data['data'] = dataJogo;
       	var escudo1 = StrToURL(d.rss.channel.item.dadosevento.time1);
       	// escudo1 = escudo1.;
      	data['escudo_1'] = 'http://e.imguol.com/futebol/brasoes/40x40/'+escudo1.toLowerCase()+'.png';
      	var escudo2 = StrToURL(d.rss.channel.item.dadosevento.time2);
